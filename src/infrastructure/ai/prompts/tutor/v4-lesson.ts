/**
 * v4 of the Tutor *lesson* prompt.
 *
 * v2 added the "visual example" guidance, v3 added a short Markdown-table
 * specification. v4 adds a second visual example family — eigenvalues and
 * eigenvectors — so a linear-algebra lesson can carry one explicit, verifiable
 * 2×2 example that the visualization step may reuse verbatim.
 *
 * Changing this prompt changes `prompts.tutorLesson.VERSION`, which is folded
 * into `TutorLesson.contentHash`; existing cached lessons therefore regenerate
 * once on their next open. `TUTOR_LESSON_VERSION` is deliberately untouched.
 */

import { securityFooter, untrustedContentWrapper } from '../security'

export const VERSION = 'v4' as const
...
export function buildSystemPrompt(): string {
  return [
    ...
    '',
    'VISUAL EXAMPLES (only when the topic suits a diagram):',
    '  - A separate step may draw a diagram from the lesson. To make that possible, include ONE short, self-contained, numerically explicit example when the topic fits:',
    '      * Linear transformation: give an explicit 2×2 matrix and an explicit 2D vector, then state the product. Example: A = [[2, 0], [0, 1]], v = (1, 1), so Av = (2, 1); the unit square becomes a rectangle of twice the area.',
    '      * Eigenvalues / eigenvectors: give an explicit 2×2 real matrix, state its real eigenvalues and at least one explicit eigenvector, and verify Av = λv. Example: A = [[2, 1], [1, 2]], λ1 = 3 with v1 = (1, 1) because Av1 = (3, 3) = 3v1, and λ2 = 1 with v2 = (1, -1). Then add one short geometric observation phrased as a statement, for example "Notice that Av is a multiple of v", never as a question.',
    '      * Set theory: give 2–3 explicit finite sets with their elements and one operation with its result. Example: A = {1, 2, 3}, B = {3, 4, 5}, so A ∩ B = {3}.',
    '      * Vectors: give explicit vector components and any sum, scalar multiple or linear combination.',
    '      * Graphs: state the vertices and edges explicitly (at most 12).',
    '  - Keep the example short and use real numbers so it can be reused verbatim. Do not add extra examples just to have a diagram.',
    '  - If the matrix in an eigenvalue example has complex (non-real) eigenvalues, say so in words and do not present a real eigen-direction. Never invent a real eigenvector for a complex eigenvalue.',
    '  - If the topic does not suit a diagram, do not force one; the lesson is complete without it.',
    '',
    'MARKDOWN TABLES:',
    '  - When content is naturally tabular — truth tables, comparisons of properties, symbol or formula reference — use a standard multi-line GitHub-style pipe table.',
    '  - A table MUST have a header row and a delimiter row, each row on its own line, with a blank line before and after the table. Example:',
    '      | P | Q | P \\wedge Q |',
    '      | :---: | :---: | :---: |',
    '      | T | T | T |',
    '      | T | F | F |',
    '      | F | T | F |',
    '      | F | F | F |',
    '  - Keep the number of columns the same on every row.',
    '  - Do NOT output an HTML `<table>`. Do NOT wrap the table in a code fence. Do NOT squash the whole table onto one line.',
    '  - Use LaTeX for ordinary formulas as usual, including inside a table cell.',
    '  - Do not add a table to every lesson; only when it genuinely helps.',
    '',
  ...
  return [
    ...
    input.sourceSnippets.length > 0
      ? untrustedContentWrapper(
          'PRIMARY COURSE MATERIAL (TEXTBOOK)',
          input.sourceSnippets.join('\n\n'),
        )
