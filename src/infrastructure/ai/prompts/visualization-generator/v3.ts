/**
 * v3 visualization-generator prompt.
 *
 * Extends v2 with `transform_2d` (linear algebra) and `venn_2d` (set theory),
 * and — most importantly — a hard rule that a diagram may only reuse the exact
 * matrix, vectors, sets and operation that already appear in the finished
 * lesson. v1 and v2 are kept unchanged; the registry points at v3.
 *
 * The model still returns semantics only. It never returns coordinates, SVG,
 * HTML, JavaScript, CSS, colours, determinants, transformed vectors or set
 * results; all of those are computed locally.
 */
...
export function buildSystemPrompt(): string {
  return [
    ...
    '  - NOT supported (omit; never approximate): implicit curves such as "x^2 + y^2 = 4" or "xy = 1"; products of two variables; tan / sec / csc / cot; absolute value; piecewise, parameterised, polar, 3D, vector-field or geometry content.',
    '',
    'transform_2d — rules:',
    ...
    JSON.stringify(
      {
        visualizations: [
          {
            type: 'function_2d | equation_2d | inequality_2d | points_2d | table_2d | vectors_2d | graph_2d | transform_2d | venn_2d',
